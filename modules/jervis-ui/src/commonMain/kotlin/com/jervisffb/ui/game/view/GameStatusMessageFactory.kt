...
class GameStatusMessageFactory(private val state: Game) {
    ...
    private val messageFactories = mutableMapOf<Node, (isActiveClient: Boolean, serverDiceRolls: Boolean) -> String?>(
        AccuracyRoll.RollDie to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient && !serverDiceRolls) -> "Roll D6 to Pass the Ball"
                else -> null
            }
        },
        AccuracyRoll.ChooseReRollSource to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient) -> "Accept Pass Result or Reroll D6?"
                else -> null
            }
        },
        AccuracyRoll.ReRollDie to { isActiveClient, serverDiceRolls ->
            when {
        ...
        Bounce.RollDirection to { isActiveClient, serverDiceRolls ->
            when {
        ...
        CatchRoll.RollDie to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient && !serverDiceRolls) -> "Roll D6 to Catch the Ball"
                else -> null
            }
        },
        CatchRoll.ChooseReRollSource to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient) -> "Accept Catch Result or Reroll D6?"
                else -> null
            }
        },
        CatchRoll.ReRollDie to { isActiveClient, serverDiceRolls ->
            when {
        ...
        DeviateRoll.RollDice to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient && !serverDiceRolls) -> "Roll D8 and D6 to Deviate the Ball"
                else -> "Deviate Roll"
            }
        },

        DodgeRoll.RollDie to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient && !serverDiceRolls) -> "Roll D6 to Dodge"
                else -> null
            }
        },
        DodgeRoll.ChooseReRollSource to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient) -> "Accept Dodge Result or Reroll D6?"
                else -> null
            }
        },
        DodgeRoll.ReRollDie to { isActiveClient, serverDiceRolls ->
            when {
        ...
        TheKickOff.PlaceTheKick to { isActiveClient, serverDiceRolls ->
            when (isActiveClient) {
                true -> "Place the Kick"
                false -> "Kick is being placed"
            }
        },
        TheKickOff.TheKickDeviates to { isActiveClient, serverDiceRolls ->
            "The Kick Deviates"
        },


        PickupRoll.RollDie to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient && !serverDiceRolls) -> "Roll D6 to Pickup the Ball"
                else -> null
            }
        },
        PickupRoll.ChooseReRollSource to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient) -> "Accept Pickup Result or Reroll D6?"
                else -> null
            }
        },
        PickupRoll.ReRollDie to { isActiveClient, serverDiceRolls ->
            when {
        ...
        RushRoll.RollDie to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient && !serverDiceRolls) -> "Roll D6 to Rush"
                else -> null
            }
        },
        RushRoll.ChooseReRollSource to { isActiveClient, serverDiceRolls ->
            when {
                (isActiveClient) -> "Accept Rush Result or Reroll D6?"
                else -> null
            }
        },
        RushRoll.ReRollDie to { isActiveClient, serverDiceRolls ->
            when {
        ...
        ScatterRoll.RollDice to { isActiveClient, serverDiceRolls ->
            "Scatter Roll"
        },
    ...
    fun applyMessage(
        ...
        while (currentNode != null && message == null) {
            ...
            if (messageFactory != null) {
                message = messageFactory(isActiveCoach, serverDiceRolls)
            }
            if (message == null) {
                currentIndex -= 1
