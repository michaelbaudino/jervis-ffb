 ...
 *
 * 5. Follow up or not. If yes, the attacker is moved. The order between Fend and
 *    Taunt is arbitrary. Right now, we have an ActionNode for each, as we
 *    assume that the chance of a player having both is extremely low. If this
 *    assumption is proven wrong, we should probably combine them into a single
 *    node as they are mutually exclusive and it would make the UX better.
 *    a. Check for Fend
 *    b. Check for Taunt
 *    c. Check for Rooted condition (disallow follow-up)
 *    d. Check for Frenzy (force follow-up)
 *    d. Attacker chooses to follow up or not
 *
...
object BB2025PushStepInitialMoveSequence: Procedure() {
