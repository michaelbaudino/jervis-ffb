 ...
 *
 * WARNING: Accessing this from the UI layer might result in race conditions, use [ProcedureStack]
 * instead.
 */
class MutableProcedureStack {
    ...
    private val history: MutableList<MutableProcedureState> = mutableListOf()
    ...
    /**
     * Push an existing [MutableProcedureStack] to the stack. This makes it possible to push a
     * pre-configured procedure, i.e., one that doesn't start at [Procedure.initialNode].
     */
    fun pushProcedure(procedure: MutableProcedureState) {
        history.add(procedure)
    }

    /**
     * Push a new [Procedure] to the stack. This will create a new [MutableProcedureState].
     */
    fun pushProcedure(procedure: Procedure) {
        history.add(MutableProcedureState(procedure, procedure.enterNode))
    }
     ...
     */
    fun popProcedure(): MutableProcedureState = history.removeLast()
     ...
     */
    fun get(index: Int): MutableProcedureState {
     ...
     */
    fun getOrNull(index: Int): MutableProcedureState? {
        if (index > 0) throw IllegalArgumentException("Index $index out of bound [0, -${history.size - 1}]")
    ...
    /**
     * Creates a read-only and thread-safe snapshot of the current state of the stack.
     */
